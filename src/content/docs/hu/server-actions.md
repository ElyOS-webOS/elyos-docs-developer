...
A Racona a SvelteKit `command` és `query` függvényeit használja szerver oldali logikához. Ezek a `*.remote.ts` fájlokban élnek, és közvetlenül hívhatók a kliens oldalról — nincs szükség külön API route-ra.
